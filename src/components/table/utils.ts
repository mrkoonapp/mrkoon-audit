...
export function localizeTableHead<T extends LocalizableHeadCell>(
  heads: T[],
  t: (key: string) => string,
  translationPrefix: string,
  keyExtractor?: (head: T) => string | undefined
): T[] {
  return heads.map((head) => {
    const key = keyExtractor ? keyExtractor(head) : head.label?.toLowerCase();
    return {
      ...head,
      label: head.unlocalized
        ? head.unlocalized
        : key
          ? t(`${translationPrefix}.${key}`) || head.label || ''
          : head.label || '',
    };
  });
}
