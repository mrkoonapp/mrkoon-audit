...
export function fDateTimeSplit(date: DatePickerFormat) {
  if (!date) {
    return { date: '-', time: '' };
  }
  ...
  // The backend sends day-first dates (DD-MM-YYYY / DD/MM/YYYY) which the native
  // parser misreads as month-first (e.g. 02-06-2026 -> Feb 6 instead of Jun 2).
  // That misparse is still "valid", so we can't rely on d.isValid() — always
  // normalize a leading day-first segment to ISO (YYYY-MM-DD) before parsing.
  if (typeof date === 'string') {
    const match = date.match(/^(\d{2})[-/](\d{2})[-/](\d{4})(.*)$/);
    if (match) {
      const [, dd, mm, yyyy, rest] = match;
      d = dayjs(`${yyyy}-${mm}-${dd}${rest}`);
    }
  }
  ...
  return {
    date: d.format(formatStr.date),
    time: d.format(formatStr.time),
  };
}
