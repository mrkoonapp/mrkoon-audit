...
/** Insert "Cairo Variable" after the primary font name for Arabic character support */
function withArabicFallback(fontStack: string): string {
  const commaIndex = fontStack.indexOf(',');
  if (commaIndex === -1) return `${fontStack}, "Cairo Variable"`;
  return `${fontStack.slice(0, commaIndex)}, "Cairo Variable"${fontStack.slice(commaIndex)}`;
}

const primaryFont = withArabicFallback(setFont(themeConfig.fontFamily.primary));
const secondaryFont = withArabicFallback(setFont(themeConfig.fontFamily.secondary));
